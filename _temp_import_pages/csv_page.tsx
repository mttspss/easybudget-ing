"use client"; // Necessario per useState, useEffect e event handlers

import { useState, useEffect } from "react";
import Papa, { ParseResult } from "papaparse";
import { Button } from "@/components/ui/button"; // Importato Button

// Definiamo un tipo per le righe parsate quando header: true
interface CsvRow {
  [key: string]: string;
}

// Aggiunta interfaccia per la mappatura delle colonne
                ...
                {(['sign', 'separate_columns', 'type_column'] as TypeDeterminationStrategy[]).map(strat => (
                  <label key={strat} className="inline-flex items-center mt-2 sm:mt-0 cursor-pointer">
                    ...
                    <span className="ml-2 text-sm text-gray-700">
                      {strat === 'sign' && 'Use amount sign (+/-) from a single amount column'}
